...
class TestDistance(unittest.TestCase):
  ...
  def test_no_nans_on_zero_vectors(self):
    """Cosine distance calculation involves a divide-through by vector
    magnitude which can divide by zeros to occur."""
    # Create some dummy data with easily verifiable distances
    q = 1  # 1 query per class
    k = 3  # 3 way classification
    d = 2  # embedding dimension of two
    ...
    self.assertTrue(
        torch.isnan(distances).sum() == 0,
        'Cosine distances between 0-vectors should not be nan')


class TestAutogradGraphRetrieval(unittest.TestCase):

  def test_retrieval(self):
    """Create a simple autograd graph and check that the output is what is
    expected."""
    ...
    self.assertEqual(
        set(expected_nodes), set(n.__class__.__name__ for n in nodes),
    ...
    expected_edges = [
        ...
        ('AccumulateGrad',
         'AddBackward0'),  # x = torch.ones(2, 2, requires_grad=True), y = x + 2
    ]
    for e in edges:
      self.assertIn(
          (e[0].__class__.__name__, e[1].__class__.__name__), expected_edges,
    ...
    for e in edges:
      if e[0].__class__.__name__ == 'AddBackward0' and e[
          1].__class__.__name__ == 'MulBackward0':
        num_y_squared_edges += 1

    self.assertEqual(
        num_y_squared_edges, 2,
