...
class TestMAML(unittest.TestCase):
  ...
  def _count_named_nodes(self, nodes: List[torch.autograd.Function],
                         name: str) -> int:
    """Counts the number of autograd graph nodes that are named in a particular
    way.
  ...
  def test_first_order(self):
    """Test the 1st order MAML is only perform a first order update by
    inspecting the autograd graph."""
    # Run a single meta batch using 1st order MAML
    nodes_1_1, _ = self._get_maml_graph(order=1, inner_train_steps=1)

    self.assertEqual(
        self._count_named_nodes(nodes_1_1, 'NllLossBackwardBackward'), 0,
        'The autograd graph of 1st order MAML should not contain any double backwards operations'
    )

  def test_second_order(self):
    """Test that 2nd order MAML is genuinely performing a second order update
    by inspecting the autograd graph."""
    # Run a single meta batch using 1st order MAML
    nodes_1_1, _ = self._get_maml_graph(order=1, inner_train_steps=1)
    ...
    self.assertGreater(
        len(nodes_2_1), len(nodes_1_1),
        '2nd order MAML should produce a larger autograd graph than 1st order MAMl'
    )

    self.assertEqual(
        self._count_named_nodes(nodes_2_1, 'NllLossBackwardBackward'), 1,
    ...
    self.assertEqual(
        self._count_named_nodes(nodes_2_2, 'NllLossBackwardBackward'), 2,
