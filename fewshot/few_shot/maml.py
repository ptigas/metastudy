...
def meta_gradient_step(model: Module, optimiser: Optimizer, loss_fn: Callable,
  ...
  for meta_batch in x:
    ...
    gradients = torch.autograd.grad(loss,
                                    ...
                                    create_graph=create_graph)
    named_grads = {
        name: g for ((name, _), g) in zip(fast_weights.items(), gradients)
    }
  ...
  if order == 1:
    if train:
      sum_task_gradients = {
          k: torch.stack([grad[k] for grad in task_gradients
                         ]).mean(dim=0) for k in task_gradients[0].keys()
      }
      hooks = []
      for name, param in model.named_parameters():
        hooks.append(param.register_hook(replace_grad(sum_task_gradients,
                                                      name)))
      ...
      logits = model(
          torch.zeros((k_way,) + data_shape).to(device, dtype=torch.double))
