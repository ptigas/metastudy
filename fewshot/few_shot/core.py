    ...
    for _ in range(self.episodes_per_epoch):
      ...
      for task in range(self.num_tasks):
        if self.fixed_tasks is None:
          ...
          episode_classes = self.fixed_tasks[self.i_task %
                                             len(self.fixed_tasks)]
          self.i_task += 1
...
class EvaluateFewShot(Callback):
  """Evaluate a network on  an n-shot, k-way classification tasks after every
  epoch.
  ...
  def on_epoch_end(self, epoch, logs=None):
    ...
    for batch_index, batch in enumerate(self.taskloader):
      ...
      totals[self.metric_name] += categorical_accuracy(y,
