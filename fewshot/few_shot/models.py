...
class GlobalMaxPool1d(nn.Module):
  ...
  def forward(self, input):
    return nn.functional.max_pool1d(input, kernel_size=input.size()[2:]).view(
        -1, input.size(1))


class GlobalAvgPool2d(nn.Module):
  """Performs global average pooling over the entire height and width of a
  batched 2D tensor.
  ...
  def forward(self, input):
    return nn.functional.avg_pool2d(input, kernel_size=input.size()[2:]).view(
        -1, input.size(1))


def conv_block(in_channels: int, out_channels: int) -> nn.Module:
  """Returns a Module that performs 3x3 convolution, ReLu activation, 2x2 max
  pooling.
...
def functional_conv_block(x: torch.Tensor, weights: torch.Tensor,
  ...
  """Performs 3x3 convolution, ReLu activation, 2x2 max pooling in a functional
  fashion.
...
def get_few_shot_resnet_encoder() -> nn.Module:
  """Creates a few shot Resnet50 based encoder as used in Matching and
...
class FewShotClassifier(nn.Module):
  ...
  def functional_forward(self, x, weights):
    """Applies the same forward pass using PyTorch functional operators using a
    specified set of weights."""
...
class MatchingNetwork(nn.Module):
  ...
  def __init__(self, n: int, k: int, q: int, fce: bool, num_input_channels: int,
    ...
    if self.fce:
      self.g = BidrectionalLSTM(lstm_input_size,
                                lstm_layers).to(device, dtype=torch.double)
...
class BidrectionalLSTM(nn.Module):
  ...
  def __init__(self, size: int, layers: int):
    """Bidirectional LSTM used to generate fully conditional embeddings (FCE)
...
class AttentionLSTM(nn.Module):
  ...
  def __init__(self, size: int, unrolling_steps: int):
    """Attentional LSTM used to generate fully conditional embeddings (FCE) of
    the query set as described in the Matching Networks paper.
